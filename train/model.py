...
class JumpModel:
    ...
    def forward(self, img, is_training, keep_prob, name):
        with tf.name_scope(name):
            with tf.variable_scope(name):
                out = self.conv2d('conv1', img, [3, 3, 1, 8], 2)
                ...
                out = tf.reshape(out, [-1, 64 * 8 * 8])
                out = self.make_fc('fc1', out, [64 * 8 * 8, 64], keep_prob)
                out = self.make_fc('fc2', out, [64, 1], keep_prob)
...
if __name__ == '__main__':
    ...
    is_training = True
    keep_prob = 0.75
    out = model.forward(tf.zeros((1, 128, 128, 1)), is_training, keep_prob, 'coarse')
